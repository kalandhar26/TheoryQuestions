...
| Purpose              | Represents a thread of execution               | Represents a task to be executed        |
| Implementation       | Implements Runnable                            | Functional interface (run() method)     |
| Thread Creation      | Extend Thread class                            | Implement Runnable and pass to Thread   |
| Flexibility          | Less (cannot extend another class)             | More (can extend other classes)         |
| Scheduling           | Managed by JVM (via Thread)                    | Not responsible for scheduling          |
| Lifecycle Management | Provides lifecycle methods (start, join, etc.) | No lifecycle control                    |
| Methods Available    | start(), sleep(), join(), yield(), interrupt() | Only run() method                       |
| Resource Sharing     | Less preferred                                 | Better for sharing tasks across threads |
| Best Practice        | Rarely used directly                           | Preferred approach in real applications |
...
- Java provides ExecutorService, which is a high-level tool to manage thread pools. 
- It allows you to track task progress using Future objects for asynchronous operations. Submit tasks easily. Shut down threads properly
...
- A thread pool is a group of worker threads that are ready to perform tasks. Instead of creating a new thread every
  time you have a job, you take one from the pool. This saves time and resources.
...
- Why? Thread Class already implementing Runnable interface. again if implement runnable it adds nothing.

## Why wait() and notify() are in Object class not in Thread class?

- In Java: Every object has i) A monitor (intrinsic lock) ii)A wait set (threads waiting on that object)
- You are locking that object’s monitor. So waiting and notifying must also happen on that object.
- If wait() were in Thread, this would break the model. Because threads don’t own locks. Objects do.
- Thread = execution, Object = monitor (lock + condition queue)
- So Thread executes, Object coordinates, That’s the architecture.

## Difference between Concurrency and Parallelism?

- Concurrency means dealing with multiple tasks at once. It does NOT mean they are running at the same time.
- The system can handle multiple tasks by switching between them.
- One CPU core. Multiple threads. OS switches between them rapidly.
- Only one thread runs at any instant, but progress appears simultaneous.
- Parallelism means multiple tasks literally running at the same time.
- Multiple CPU cores Or multiple processors. True simultaneous execution. No switching required.
...
| Actual simultaneous execution? | ❌ Not necessarily | ✅ Yes                 |
| Focus                          | Managing tasks    | Speeding up execution |
| Main goal                      | Responsiveness    | Performance           |
