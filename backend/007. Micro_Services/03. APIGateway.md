...
spring:
  ...
  cloud:
    gateway:
      # Enables detailed logging of routes & filters (useful in dev/test)
      logging:
        enabled: true   # or set via --debug / logging.level.org.springframework.cloud.gateway=DEBUG

      # Global timeouts (recommended for production)
      httpclient:
      ...
      # Enables automatic route registration from service discovery (Eureka/Consul)
      # discovery:
      #   locator:
      #     enabled: true
      ...
      routes:
        - id: customer-service
          uri: http://localhost:8081          # Later → lb://customer-service
          predicates:
            - Path=/customers/**
          filters:
            - StripPrefix=1                     # Removes /customers from downstream request
            - name: RequestRateLimiter
              args:
                redis-rate-limiter.replenishRate: 10
                redis-rate-limiter.burstCapacity: 20
                redis-rate-limiter.requestedTokens: 1
                key-resolver: "#{@userKeyResolver}"   # Better: use principal name or IP
            - name: Retry
              args:
                retries: 3
                statuses: 503, 504                 # Service Unavailable, Gateway Timeout
                methods: GET, HEAD                 # Only retry safe methods
                backoff:
                  firstBackoff: 50ms
                  maxBackoff: 500ms
                  factor: 2
            ...
            - name: CircuitBreaker
              args:
                ...
                statusCodes: 500-599               # More explicit
        ...
        - id: loan-service
          ...
          filters:
            ...
            - AddRequestHeader=X-Service, loan

      # Global filters — applied to **every** route
      default-filters:
        - AddResponseHeader=X-Response-Time, "#{T(java.time.LocalDateTime).now()}"
        - name: JwtAuthenticationFilter          # Your custom JWT filter

        # Add global CORS (very common need)
        - name: Cors
          args:
        ...
        # Optional: global request logging / tracing
        - name: AddRequestHeader
          args:
            name: X-Request-ID
            value: "#{T(java.util.UUID).randomUUID().toString()}"

        # Optional: DedupeResponseHeader (removes duplicate headers from downstream)
        - DedupeResponseHeader=Access-Control-Allow-Origin Access-Control-Allow-Methods, RETAIN_FIRST

      # Optional: Enable detailed actuator endpoints for monitoring
      # management:
      #   endpoints:
      #     web:
      #       exposure:
      ...
      # Rate limiter key resolver bean (example — add this in a @Configuration class)
      # @Bean
