...
- Storing frequently accessed data in memory to reduce latency and load on downstream systems.
...
- Redis → dominant (low latency, distributed)
- Ehcache → legacy monolith apps
- Hazelcast → cluster-based caching

## Blunt Gaps

- Cache inconsistency → wrong balances (critical issue)
- Cache eviction strategy often ignored
- Over-caching → stale data → regulatory risk
