...
| Aspect     | Class                 | Object                        |
|------------|-----------------------|-------------------------------|
| Definition | Blueprint or template | Real instance of a class      |
...
| Decision  | Decided at Compile Time   | Decided at Runtime   |
...
- Yes, it’s possible. imagine a PaymentProcessor class that is perfectly cohesive (all methods relate to processing
  payments), but it directly depends on concrete classes like BankAPI, EmailNotifier, and Logger. It’s cohesive in
  purpose, but tightly coupled because it cannot function without those specific external classes.
- Cohesion is about internal focus, while coupling is about external dependencies. They are independent qualities.

### 80. Is it possible to increase cohesion and coupling at the same time?

- Yes, it can happen.
  When you refactor a class to make it more cohesive (removing unrelated responsibilities), you might also introduce
  stronger dependencies on other classes to delegate those responsibilities. That delegation can increase coupling.
- You split a UserManager class into UserValidator, UserRepository, and UserNotifier.
- Each class is now more cohesive.
- But they now depend on each other more explicitly, which increases coupling.
- Best practice: Aim for high cohesion + low coupling by introducing abstractions (interfaces, dependency injection) so
  that classes remain focused but not rigidly bound to each other.

### 81. How does java handle static and dynamic binding?

- Static binding also known as early binding. This happens when the method is resolved at compile time. It is used for
  methods that are masked as final or private, which cannot be overridden.
- Dynamic binding also known as late binding. This happens when the method is resolved at runtime. The method is
  executed based on objects runtime type.
- Virtual Method Invocation : In Java, non-static methods are by default called "virtual functions" . This means the
  method implementation that gets invoked is one belonging to runtime type of object, not type of reference.
